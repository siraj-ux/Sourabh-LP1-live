        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="text-center md:text-left">
            ...
            <h1 className="text-xl sm:text-2xl md:text-3xl font-black leading-tight mb-4">
              <span className="text-gradient">Spine, Knee & Neck Pain</span>{" "}
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left">
              <div className="relative inline-block group mt-2">
                <div className="absolute -inset-1 rounded-xl bg-gradient-to-r 
                                ...
                                transition duration-1000 animate-gradient-x"></div>
                <button
                  onClick={() => {
                  ...
                  active:scale-95"
                >
                  BOOK NOW — Sirf <span className="relative">41 Seats</span> Bachi Hain!
                </button>
              </div>

              <p className="text-sm font-semibold mt-6">Full Hone Par Booking Band!</p>
              ...
              <div className="flex gap-4 mt-6 justify-center md:justify-start">
                {[
                  ...
                  <div key={i} className="flex flex-col items-center">
                    ...
                    <span className="text-[10px] mt-2 text-muted-foreground">{t.label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex justify-center">
            <div 
              className="relative group cursor-pointer rounded-2xl overflow-hidden shadow-glow border border-border bg-card w-full max-w-[400px]"
              onClick={togglePlay}
            >
              <video
                ref={videoRef}
              ...
              {/* Center Play Button Overlay (Visible when paused or hovered) */}
              <div className={`absolute inset-0 flex items-center justify-center bg-black/20 transition-opacity duration-300 ${isPlaying ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'}`}>
                <div className="bg-orange-500/90 p-5 rounded-full shadow-[0_0_30px_rgba(255,165,0,0.8)] transform transition-transform group-hover:scale-110">
                  {isPlaying ? (
              ...
              {/* Small "Click for Sound" badge when muted */}
              {isMuted && isPlaying && (
                <div className="absolute bottom-16 right-4 bg-orange-500 text-black text-[10px] font-bold px-2 py-1 rounded animate-bounce">
                  CLICK FOR SOUND
                </div>
              )}
            </div>
